...
6. **Kernel minidump analysis** — parses PAGEDU64/MDMP crash dumps to extract bugcheck codes and parameters, identifies GPU-related crashes (VIDEO_TDR_FAILURE, VIDEO_TDR_TIMEOUT_DETECTED, VIDEO_SCHEDULER_INTERNAL_ERROR) — and, when cdb deep analysis is available, also flags crashes whose faulting module is in the graphics stack (the NVIDIA driver or the DirectX graphics kernel) even under a generic bugcheck (e.g. an `0x7F` trap bucketed to `nvlddmkm`)
...
Grab the latest build from the [GitHub Releases page](../../releases). Single `FLARE.exe`, no installer. Requires the [.NET 10 Desktop Runtime](https://dotnet.microsoft.com/download/dotnet/10.0). CI builds binaries only for release tags, so Releases is the only download channel; for anything in between, build from source.

The title bar tells you which kind of build you're running: a clean `FLARE <version> - …` is an official release — only a build produced by CI from a `v*` tag carries the release brand, so a local build (even one sitting on the tag commit) never shows as clean. `[SNAPSHOT] FLARE <version>-alpha.0.<N>+<hash> - …` is a non-release build carrying a commit hash (e.g. a local `build-exe.ps1`; the `-alpha.0.<N>` suffix is MinVer's commit height past the most recent `v*` tag). `[DEV BUILD] FLARE <version>+dev - …` is a plain local `dotnet build`. The About dialog repeats this information.
